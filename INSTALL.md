...
+ the isolated per-family venvs described below, so every feature works out
of the box — **no containers required**. A successful full run ends with
`13 Ready, 0 Missing` in the model import probe. Subsequent runs skip
already-satisfied installs via fast `python -c "import …"` probes.

#### Isolated per-family venvs (`.cache/`)

Three sidecars pin dependency sets that cannot coexist with the main
interpreter. Docker gives each its own image; on bare metal `./run.sh --all`
mirrors that with one `uv`-managed venv per family, created automatically:

| Family | Venv | Why it can't share the main interpreter |
|---|---|---|
| `basic-pitch`, `autochord` | `.cache/py311-cue/` (Python 3.11) | `tensorflow<2.15.1` / keras<3 have no wheels for Python 3.12+ |
| `msaf` | `.cache/msaf-venv/` (Python 3.10) | msaf imports `np.int` / `np.float`, removed in numpy 1.24, while `requirements.txt` needs `numpy>=1.24` |

`uv` is fetched automatically if missing (~30 MB static binary, no system
deps), and it downloads its own prebuilt CPython for each version. The msaf
venv pins the exact versions `docker/msaf.Dockerfile` uses, so bare metal and
Docker run identical code.

You do not need to do anything for this — it is what `--all` does. Two notes
if you are debugging it:

- Each venv installs in **two stages** (build prerequisites, then the payload
  with `--no-build-isolation`). `vamp`, pulled in by autochord, calls numpy at
  build time without declaring it, so a single atomic install of the family
  fails and takes `basic-pitch` down with it.
- A family is considered usable only when its modules actually **import**, not
  when the installer exits 0. A venv left healthy by an earlier run is reused
  even on a lean `./run.sh`, so you do not have to re-run `--all` to get your
  sidecars back.

Logs land in `/tmp/uv_install_<family>.log` and `/tmp/uv_venv_<family>.log`.
...
# MSAF (boundary algos sf / foote / cnmf / olda) — do NOT install this into
# the main interpreter: it needs numpy<1.24 and requirements.txt needs >=1.24,
# so pip will either refuse or leave you with a broken numpy. Give it its own
# Python 3.10 env (this is what ./run.sh --all automates):
uv venv --python 3.10 --seed .cache/msaf-venv
uv pip install --python .cache/msaf-venv/bin/python \
  "numpy>=1.20,<1.24" "setuptools<81" wheel "Cython>=0.29"
uv pip install --python .cache/msaf-venv/bin/python --no-build-isolation \
  "scipy>=1.7,<1.11" "librosa>=0.9.0,<0.10.0" "scikit-learn>=1.0,<1.3" \
  "soundfile>=0.10.0" "audioread>=3.0.0" "msaf>=0.1.80"
# then run the server on that interpreter:
#   .cache/msaf-venv/bin/python tools/python/msaf_server.py
