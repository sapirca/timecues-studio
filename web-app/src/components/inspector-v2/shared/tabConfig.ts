 ...
 * source for the `'boundaries'` tab.
 */

import type { AnnotationLayerType } from '../../../types/annotationLayer';

/** Top-level annotation tabs the editor exposes. Identical to the storage
 *  union by construction — every tab is a multi-layer type backed by
 *  AnnotationLayersDocument, so the two can never drift. `boundaries` also
 *  carries a source (manual/autoGuess/detector) picked in the
 *  AnnotationSourcePicker dropdown, where `manual` means "the selected
 *  boundary layer". */
export type AnnotationType = AnnotationLayerType;
...
export const TAB_CONFIG: TabLeaf[] = [
  ...
  { id: 'riff-patterns', label: 'Riff Patterns', experimental: 'loopsAndPatterns' },
];
...
/** The kinds that have a detector/evaluation pipeline behind them. Riff
 *  patterns are composed by hand in the riff window out of nodes the user
 *  authors — no detector emits them and no ground truth scores them — so they
 *  are excluded from the Examine picker, the Evaluation tables and detector
 *  review. Matches `EvalTabKey` in EvaluationStage and `ReviewableCategory` in
 *  detectorConvert; keep the three in step. */
export type ExaminableType = Exclude<AnnotationType, 'riff-patterns'>;

/** Narrows an annotation type to the kinds a detector can produce and the
 *  Evaluation stage can score. */
export function isExaminableType(type: AnnotationType): type is ExaminableType {
  return type !== 'riff-patterns';
}

/** The multi-layer annotation kinds that share the AnnotationLayersDocument.
 *  Every annotation type is one — boundaries joined the rest when the
 *  single-doc `ManualAnnotation` was retired. Single source of truth:
 *  `isLayerType` and `supportsRangePending` both derive from this. */
const LAYER_TYPES = new Set<AnnotationType>(['boundaries', 'cues', 'spans', 'loops', 'riff-patterns', 'lyrics']);

/** True when the type is one of the multi-layer annotation kinds that share
 *  the AnnotationLayersDocument. */
export function isLayerType(type: AnnotationType): type is AnnotationLayerType {
  return LAYER_TYPES.has(type);
}

/** True for the types that participate in the drag-range "+ Add" pending pill.
 *  - Layer kinds with a real t1/t2 span: Spans/Loops (always).
 *  - Cues: a range drops ONE point-cue at its start (cues have no duration);
 *    the panel warns that the region's end was discarded.
...
export function supportsRangePending(
  ...
  // AutoGuess is review-only: it never accepts a drag-range.
