...
interface KaraokePanelProps {
  ...
  color?: string;
  /** Id of the layer being shown — the picker's current value. */
  layerId?: string;
  /** Every lyrics layer that has words. A picker appears when there's >1. */
...
export function KaraokePanel({
  ...
  layerId, layerOptions, onLayerChange,
}: KaraokePanelProps) {
  const picker = { layerId, layerOptions, onLayerChange };
  ...
  if (!lines.length) {
    return (
      <div className="rounded-lg border border-sky-400/20 bg-sky-500/[0.04] p-4">
        <Header title={title} color={color} {...picker} />
  ...
  return (
    <div className="rounded-lg border border-sky-400/20 bg-sky-500/[0.04] p-4">
      <Header title={title} color={color} lineNo={activeIdx + 1} total={lines.length} {...picker} />
...
function Header({
  title, color, lineNo, total, layerId, layerOptions, onLayerChange,
}: {
  title: string;
  color: string;
  lineNo?: number;
  ...
  // With a single lyrics layer the name is a label; with several it's a
  // picker, because otherwise the only way to switch is to click a word in
  // the layer you want (which is what the panel falls back to).
  const canPick = !!onLayerChange && (layerOptions?.length ?? 0) > 1;
  return (
    <div className="flex items-center justify-between gap-3">
      <h3 className="text-[12px] font-semibold flex items-center gap-1.5 min-w-0" style={{ color }}>
        ...
        {canPick ? (
          <select
            ...
            title="Which lyrics layer the karaoke view follows"
            className="max-w-[220px] h-5 text-[12px] font-semibold rounded border border-white/[0.12] bg-white/[0.03] px-1 py-0 leading-none cursor-pointer focus:outline-none hover:bg-white/[0.07] transition-colors"
            style={{ color }}
          >
            {layerOptions!.map((l) => (
