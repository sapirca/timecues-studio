...
export function SetlistPage() {
  ...
  return (
    <div className="px-4 sm:px-3 pt-3 pb-12 text-slate-200">
      <div className="max-w-6xl mx-auto space-y-4">
        ...
        <section className="rounded-lg border border-white/[0.06] bg-[#14171d]/80 p-3 space-y-3">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <select
              ...
              className="bg-[#0e1015] border border-white/[0.08] rounded px-2 py-2 sm:py-1 text-base sm:text-[12px] text-slate-200"
            >
              {STRATEGIES.map((s) => (
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              ...
              className="px-3 py-2.5 sm:py-1.5 rounded text-[12px] font-medium bg-rose-500/20 text-rose-200 border border-rose-400/40 hover:bg-rose-500/30 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
            >
              Generate order
            </button>
            <button
              onClick={() => { setGenerated(null); setStatus(null); }}
              className="px-3 py-2.5 sm:py-1.5 rounded text-[12px] text-slate-400 hover:text-slate-200 hover:bg-white/[0.04] transition-colors"
            >
              Reset
            </button>
        ...
        <section className="rounded-lg border border-white/[0.06] bg-[#14171d]/80 p-3 flex flex-wrap items-center gap-2">
          ...
          <input
            ...
            className="bg-[#0e1015] border border-white/[0.08] rounded px-2 py-2 sm:py-1 text-base sm:text-[12px] text-slate-200 min-w-[10rem] flex-1 sm:min-w-0 sm:flex-none sm:w-48"
          />
          <button
            onClick={handleSave}
            disabled={saving || !generated}
            className="px-3 py-2.5 sm:py-1.5 rounded text-[12px] bg-emerald-500/20 text-emerald-200 border border-emerald-400/40 hover:bg-emerald-500/30 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
          >
            {saving ? 'Saving…' : 'Save'}
          </button>
          {savedNames.length > 0 && (
            <>
              ...
              <button
                ...
                disabled={!activeName}
                className="px-2 py-2.5 sm:py-1.5 rounded text-[12px] sm:text-[11px] text-rose-300 hover:text-rose-200 hover:bg-rose-500/10 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
              >
                Delete
              </button>
            </>
          ...
          <button
            ...
            disabled={!generated}
            className="px-3 py-2.5 sm:py-1.5 rounded text-[12px] text-slate-300 hover:text-white border border-white/[0.08] hover:bg-white/[0.04] disabled:opacity-40 disabled:cursor-not-allowed transition-colors ml-auto"
          >
            Export JSON
          </button>
        </section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <section className="min-w-0 rounded-lg border border-white/[0.06] bg-[#14171d]/80 p-3">
            ...
            {loading ? (
              ...
              <ul className="max-h-[60vh] overflow-y-auto divide-y divide-white/[0.04] text-[12px]">
                {songs.map((s) => (
                  <li key={s.slug} className="flex items-center gap-2 py-2.5 sm:py-1.5">
                    <input
                      type="checkbox"
                      checked={included.has(s.slug)}
                      onChange={() => toggleSong(s.slug)}
                      className="accent-rose-400 w-4 h-4 sm:w-auto sm:h-auto shrink-0"
                    />
                    <span className="flex-1 truncate text-slate-200">{s.name}</span>
                    <span className="font-mono text-[11px] text-slate-400 tabular-nums w-16 text-right shrink-0 whitespace-nowrap">
                ...
                {generated.order.map((e, i) => (
                  <li key={e.slug} className="py-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-mono text-[11px] text-slate-500 w-10 text-right shrink-0">{e.meter ?? '—'}</span>
                    </div>
