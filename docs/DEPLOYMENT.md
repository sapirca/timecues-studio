...
### The phone address (optional)

The web app gives phones its own layout (see the user guide's
[On a phone](USER_GUIDE.md#on-a-phone)). Phones get it on your main hostname
too, but the app picks the layout from the hostname — anything starting with
`m.` is the phone one — so an `m.` hostname makes it a link you can hand out,
and keeps the desktop address free of it. (In development the same rule gives
you `http://m.localhost:5173/`, with no setup: every `*.localhost` name
resolves to 127.0.0.1 and Vite serves it.) To serve one:

1. **DNS:** point `m.your-domain.org` at the same IP (a wildcard `*` record
   covers it).
2. **Reverse proxy and host check:** the hostname needs its own site in the
   proxy — in the bundled Caddyfile that is `TIMECUES_MOBILE_DOMAIN`, which
   Caddy serves beside the main hostname and gets a certificate for — and it
   has to be listed in `VITE_ALLOWED_HOSTS` (comma-separated) or Vite answers
   403. Until the proxy has a site for the name it has nothing to present at
   the TLS handshake, which a CDN in front reports as a failed handshake
   (Cloudflare: error 525). A CDN-proxied hostname still gets a normal Let's
   Encrypt certificate — Cloudflare, for one, exempts the ACME challenge path
   from its HTTPS redirect, so the challenge reaches the origin.
3. **Google sign-in:** add `https://m.your-domain.org` to the OAuth client's
   *Authorized JavaScript origins*. It is a separate origin from the main
   hostname, so without it Google rejects the sign-in popup there.
