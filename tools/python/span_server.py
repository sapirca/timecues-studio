...
from paths import find_audio, stem_audio, cache_name, REPO_ROOT, SPAN_OUTPUTS_DIR as CACHE_DIR  # noqa: E402
...
def _err(algo: str, msg: str) -> dict:
    return _common_err(algo, msg, spans=[])
...
def detect_one(slug: str, algo: str, stem: str = "mix", force: bool = False) -> dict:
    ...
    if pitch:
        pitch_path = cache_dir / f"{cache_name(algo, stem)}.pitch.json"
        try:
