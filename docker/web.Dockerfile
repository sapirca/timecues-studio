...
# git — required by shared-song annotation history. Each collaboratively
# edited song keeps a real repo at data/annotations/shared/<slug>/.git with one
# commit per hand-off of the edit lease, so "put it back the way Alice had it"
# is `git show`, not a feature we maintain. --no-install-recommends keeps this
# to the binary and its libraries (~35 MB) instead of pulling perl tooling.
# Without it the server still runs: history falls back to timestamped copies
# (see hasGit() in web-app/server/sharedAnnotations.ts).
RUN apt-get update \
